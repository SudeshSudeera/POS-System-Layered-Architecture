...
public class ProductDetailBoImpl implements ProductDetailBo {
    ...
    public boolean saveProductDetail(ProductDetailDto dto) throws SQLException, ClassNotFoundException {
        return dao.save(
                new ProductDetail(
                        dto.getCode(),dto.getBarCode(),dto.getQtyOnHand(),dto.getSellingPrice(),dto.getShowPrice(),dto.getBuyingPrice(),dto.getProductCode(),dto.isDiscountAvailability()
                )
        );
    }
        ...
        for (ProductDetail d: dao.findAllProductDetails(productCode)){
            dtos.add(
                 new ProductDetailDto(
                         d.getCode(),d.getBarCode(),
                         d.getQtyOnHand(),d.getSellingPrice(),
                         d.getShowPrice(),d.getBuyingPrice(),
                         d.getProductCode(),d.isDiscountAvailability()
                 )
            );
        }
        return dtos;
